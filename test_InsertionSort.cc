...
// 테스트 함수
double testInsertionSort(void (*inputGen)(int*, int), int size, int repeat) {
...
int main() {
    ...
    for (int s = 0; s < 4; ++s) {
        ...
        cout << "\n===== InsertionSort | Input Size: " << size << " | Repeat: " << repeat << " =====\n";

        for (int t = 0; t < 4; ++t) {
            double avg = testInsertionSort(generators[t], size, repeat);
