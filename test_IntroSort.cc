...
double testIntroSort(void (*inputGen)(int*, int), int size, int repeat) {
...
int main() {
    ...
    for (int s = 0; s < sizeof(sizes) / sizeof(int); ++s) {
        int size = sizes[s];
        cout << "\n===== IntroSort | Input Size: " << size << " | Repeat: " << repeat << " =====\n";

        for (int t = 0; t < 4; ++t) {
            double avg_time = testIntroSort(generators[t], size, repeat);
