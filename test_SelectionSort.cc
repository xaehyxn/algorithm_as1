...
double testSelectionSort(void (*inputGen)(int*, int), int size, int repeat) {
...
int main() {
    ...
    for (int s = 0; s < 3; ++s) {
        int size = sizes[s];
        cout << "\n===== SelectionSort | Input Size: " << size << " | Repeat: " << repeat << " =====\n";
        for (int t = 0; t < 4; ++t) {
            double avg = testSelectionSort(generators[t], size, repeat);
