...
#include <cstring>
#include <algorithm>
#include "SortCode.h" // TimSort 함수 선언 포함
...
// --- 실행 시간 측정 ---
double testTimSort(void (*inputGen)(int*, int), int size, int repeat) {
    ...
    for (int i = 0; i < repeat; ++i) {
        ...
        TimSort(temp, size);  // ? 여기만 MergeSort → TimSort
...
int main() {
    ...
    for (int s = 0; s < sizeof(sizes) / sizeof(int); ++s) {
        ...
        for (int t = 0; t < 4; ++t) {
            double avg_time = testTimSort(generators[t], size, repeat);
